...
export async function POST(request: Request) {
    ...
    try {
        ...
        const epoch = Number(rows[rows.length - option - 1].epoch) + 3;
        const timeStamp = Number(rows[rows.length - option - 1].locktimestamp) + 306 * 3;
        ...
        try {
            ...
            const insert_bet_query = `
                INSERT INTO Betting (epoch, timeStamp, value)
                VALUES ($1, $2, $3);
            `;
